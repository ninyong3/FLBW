...
[Serializable]
public class AllowedPair { public string bubbleId; public string sceneKey; }
...
public class SimpleBubbleRegistry : MonoBehaviour
{
    ...
    static void EnsureInstance()
    {
        ...
        else Debug.Log("[REG] No prefab found. Waiting for scene instance.");
    }
    ...
    void Awake()
    {
        ...
        {
            if (actAsProxyWhenDuplicate) { proxyRuntime = true; Debug.Log("[REG] Duplicate acting as proxy"); return; }
            Debug.LogWarning("[REG] Duplicate destroyed (keep " + Instance.name + ")");
    ...
    { if (proxyRuntime && Instance != null && Instance != this) { Instance.ResetVisitsForNewDay(); return; }
      visitsToday.Clear(); if (verbose) Debug.Log("[REG] ResetVisitsForNewDay()"); }
    ...
    public void BeginDay(int day, string currentHeroineBubbleId, string callerTag)
    {
        ...
        { if (verbose) Debug.Log($"[REG] BeginDay ignored (same day & already assigned). caller='{callerTag}', day={day}"); return; }

        currentDay = day;
        LastCallerTag = callerTag; LastDay = day; LastCurrent = currentHeroineBubbleId;
        LastStackTrace = UnityEngine.StackTraceUtility.ExtractStackTrace();

        todayMap.Clear();

        var exclude = NormalizeKey(currentHeroineBubbleId);
        var heroines = allowedMap.Keys.Where(h => !string.Equals(h, exclude, StringComparison.OrdinalIgnoreCase))
                                      .Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        var usedScenes = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        if (verbose)
        {
            Debug.Log($"[REG] BeginDay start day={day}, exclude={exclude}, caller={callerTag}");
            foreach (var kv in allowedMap) Debug.Log($"[REG] candidates {kv.Key} -> {string.Join(", ", kv.Value)}");
        }

        foreach (var heroine in heroines)
        {
            if (!allowedMap.TryGetValue(heroine, out var candidates) || candidates.Count == 0)
            { if (verbose) Debug.Log($"[REG] {heroine}: NO CANDIDATES"); continue; }

            if (verbose) Debug.Log($"[REG] {heroine}: candidates={string.Join(", ", candidates)} used={string.Join(", ", usedScenes)}");

            var chosen = PickSceneRoundRobin(day, candidates, usedScenes, heroine);
            if (verbose) Debug.Log($"[REG] {heroine}: chosen={(string.IsNullOrEmpty(chosen) ? "NONE" : chosen)}");
        ...
        visitsToday.Clear();
        if (verbose) Debug.Log($"[REG] BeginDay result pairs = {string.Join(", ", todayMap.Select(kv => kv.Key + "->" + kv.Value))}");
    }
    ...
    { if (proxyRuntime && Instance != null && Instance != this) return Instance.CanEnterToday(sceneKey);
      var k = NormalizeKey(sceneKey); int c = visitsToday.TryGetValue(k, out var v) ? v : 0; return c < 1; }
    ...
    { if (proxyRuntime && Instance != null && Instance != this) { Instance.ReplaceAll(snapshot); return; }
      todayMap.Clear(); if (snapshot == null) return;
      foreach (var kv in snapshot) todayMap[NormalizeKey(kv.Key)] = NormalizeKey(kv.Value); }
    ...
    { if (proxyRuntime && Instance != null && Instance != this) return Instance.GetVisitCount(sceneKey);
      var k = NormalizeKey(sceneKey); return visitsToday.TryGetValue(k, out var v) ? v : 0; }
    ...
    {
        var r = Instance; if (r == null) { Debug.Log("[REG] Dump: Instance=null"); return; }
    ...
    public bool TryReassignFromDisabledScene(string disabledSceneKey, string callerTag = "Reassign")
    {
        ...
        if (!todayMap.TryGetValue(from, out var bubbleIdLower)) return false; // 그 버튼에 버블이 없으면 패스
        ...
        usedScenes.Remove(from); // 자기 자리는 비워둔다
        ...
        {
            if (verbose) Debug.Log($"[REG] Reassign failed: no free target for {bubbleIdLower} from '{from}'");
            return false; // 덮어쓰지 않음 (원하면 덮어쓰기 정책으로 변경 가능)
        }

        todayMap.Remove(from);
        todayMap[target] = bubbleIdLower;
        if (verbose) Debug.Log($"[REG] Reassign {bubbleIdLower}: {from} -> {target} (caller={callerTag})");
        return true;
    }
    // ---------- 추가 끝 ----------
    ...
    void RebuildAllowedMap()
    {
        ...
        foreach (var p in allowedPairs)
        {
            ...
            { list = new List<string>(); allowedMap[h] = list; }
            if (!list.Contains(s, StringComparer.OrdinalIgnoreCase)) list.Add(s);
        }
        if (verbose) foreach (var kv in allowedMap)
            Debug.Log($"[REG] candidates {kv.Key} -> {string.Join(", ", kv.Value)}");
    }

    public void ResetForTitle()
    {
        if (proxyRuntime && Instance != null && Instance != this) { Instance.ResetForTitle(); return; }
        todayMap.Clear(); visitsToday.Clear(); currentDay = int.MinValue;
